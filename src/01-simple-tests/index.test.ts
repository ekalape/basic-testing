...
describe('simpleCalculator tests', () => {
  ...
  test('should divide two numbers', () => {
    ...
    expect(simpleCalculator({ a: 7, b: 0, action: Action.Divide })).toBe(Infinity)
  });

  test('should exponentiate two numbers', () => {
    expect(simpleCalculator({ a: 4, b: 2, action: Action.Exponentiate })).toBe(16)
    expect(simpleCalculator({ a: 1, b: 1, action: Action.Exponentiate })).toBe(1)
    expect(simpleCalculator({ a: 0, b: 7, action: Action.Exponentiate })).toBe(0)
  });
  ...
  test('should return null for invalid arguments', () => {
    ...
    expect(simpleCalculator({ a: 8, b: "b", action: Action.Substract })).toBeNull
    expect(simpleCalculator({ a: null, b: undefined, action: Action.Exponentiate })).toBeNull
  });
});
